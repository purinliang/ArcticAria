    ...
    const renderAuthButtons = () => (
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            {isLoggedIn ? (
                ...
                <>
                    <Button variant="contained" color="primary" onClick={() => navigate("/login")} startIcon={<LoginIcon />} sx={{ textTransform: "none", borderRadius: "8px" }}>
                        {t("auth.signIn")}
                    </Button>
                </>
            )}
        ...
        <Box sx={{ width: 250 }} role="presentation" onClick={toggleDrawer(false)} onKeyDown={toggleDrawer(false)}>
            <List>
                <ListItem button onClick={() => handleNavigation("/todos")}>
                    <ListItemIcon sx={{ minWidth: "36px", color: "black" }}><ChecklistRtlIcon /></ListItemIcon>
                ...
                <ListItem button onClick={() => handleNavigation("/discover")}>
                    <ListItemIcon sx={{ minWidth: "36px", color: "black" }}><ExploreIcon /></ListItemIcon>
                ...
                <ListItem button onClick={() => handleNavigation("/blog")}>
                    <ListItemIcon sx={{ minWidth: "36px", color: "black" }}><ForumIcon /></ListItemIcon>
                    <ListItemText primary={t("nav.discuss")} primaryTypographyProps={{ sx: { fontWeight: "bold" } }} />
                </ListItem>
                <Divider sx={{ my: 1 }} />
                <ListItem button onClick={toggleLanguage}>
                    <ListItemIcon sx={{ minWidth: "36px" }}><LanguageIcon /></ListItemIcon>
                    <ListItemText primary={switchLanguageText} />
                </ListItem>
                {isLoggedIn ? (
                    <>
                        <ListItem>
                            <ListItemIcon sx={{ minWidth: "36px" }}><AccountCircleIcon /></ListItemIcon>
                        ...
                        <ListItem button onClick={handleLogout} sx={{ color: "error.main" }}>
                            ...
                            <ListItemText primary={t("auth.signOut")} primaryTypographyProps={{ sx: { fontWeight: "bold" } }} />
                        </ListItem>
                    </>
                ) : (
                    <ListItem button onClick={() => handleNavigation("/login")}>
                        <ListItemIcon sx={{ minWidth: "36px", color: "primary.main" }}><LoginIcon /></ListItemIcon>
